...
func _opts(session: C3Http.Session) -> C3Http.Options:
...
When a server sends a `Connection: close` response header, it signals that it will close the socket after this response. `C3Http` honors this: the connection is **not** returned to the pool, and the next call opens a fresh one. This prevents handing a dying connection to the next request.
